...
def train(arguments):
    ...
    for epoch in range(model.which_epoch, train_opts.n_epochs):
        ...
        for epoch_iter, (images, labels) in tqdm(enumerate(train_loader, 1), total=len(train_loader)):
            ...
            # Debug: Print errors for the first iteration
            print(f"Epoch {epoch}, Iteration {epoch_iter}: {errors}")
            break  # Remove this break after verifying the first iteration
        ...
        for split in ['train', 'validation', 'test']:
            print(f"Epoch {epoch} - {split} errors: {error_logger.get_errors(split)}")
        error_logger.reset()
