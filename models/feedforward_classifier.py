...
class FeedForwardClassifier(BaseModel):
    ...
    def get_fp_bp_time2(self, size=None):
        ...
        # Create input and output arrays on the selected device
        inp_array = Variable(torch.rand(*size)).to(self.device)
        out_array = Variable(torch.rand(*size)).to(self.device)

        # Benchmark forward and backward pass time
        fp, bp = benchmark_fp_bp_time(self.net.to(self.device), inp_array, out_array)
